<script lang="ts">
	...
	import { TIER_2_BASE_OPTIONS, TIER_3_BASE_OPTIONS } from '@domain/constants/rules';
	...
	const previously_chosen_domain_cards = $derived.by(() =>
		get_previously_chosen_domain_card_ids(characterCtx, level, [
			'tier_2_domain_card',
			'tier_3_domain_card'
		])
	);
	...
	const highlighted = $derived.by(() =>
		calculate_highlighted(characterCtx, level, ['tier_2', 'tier_3'])
	);
	let tier_2_options_open = $state(false);
	let tier_3_options_open = $state(true);
	let after_remove_secondary_class: (() => void) | undefined = $state(undefined);
	...
	<div class={cn(className)}>
		<Dropdown
			...
			subtitle={[
				...
				...subclass_level_up_options.map((option) => option.short_title)
			]
				.filter((title) => title)
				.join(', ')}
		>
			<div class="flex flex-col gap-4" bind:clientWidth={width}>
				<div class="flex flex-col gap-2 rounded-md bg-primary/50 p-2">
					...
					<DomainCardSelector
						...
						available_cards={get_available_domain_cards(characterCtx, level, 7, true)}
				...
				<Select.Root
					...
					onOpenChange={(open) => {
						if (open) {
							tier_2_options_open =
								(choices.A.option_id?.startsWith('tier_2') ?? false) ||
								(choices.B.option_id?.startsWith('tier_2') ?? false);
				...
				>
					...
					<Select.Content class="z-49 rounded-md" align="start">
				...
				{#each ['A' as keyof typeof choices, 'B' as keyof typeof choices] as key}
					{#if choices[key].option_id === 'tier_2_traits' || choices[key].option_id === 'tier_3_traits'}
						<TraitsSelector
							...
							marked_traits={derived_character_data.tier_3_marked_traits}
							{width}
						/>
					...
					{:else if choices[key].option_id === 'tier_2_domain_card' || choices[key].option_id === 'tier_3_domain_card'}
						...
						<div class="flex flex-col gap-2 rounded-md bg-primary/50 p-2">
							...
							<DomainCardSelector
								...
								available_cards={get_available_domain_cards(characterCtx, level, max_level, true)}
								previously_chosen_card_ids={previously_chosen_domain_cards}
					...
					{:else if choices[key].option_id === 'tier_3_subclass_upgrade'}
						<SecondarySubclassSelector
							...
							options_used={derived_character_data.level_up_options_used}
							{width}
						/>
						{#each subclass_level_up_options.filter((option) => option.type === 'domain_card' && option.subclass_side === choices[key].selected_subclass_upgrade) as option (option.option_id)}
						...
						<SecondaryClassSelector
							after_remove_secondary_class={() => {
								if (after_remove_secondary_class) {
									after_remove_secondary_class();
									after_remove_secondary_class = undefined;
								}
							}}
						/>
					{/if}
