<script lang="ts">
	...
	const highlighted = $derived.by(() =>
		calculate_highlighted(characterCtx, level, ['tier_2', 'tier_3', 'tier_4'])
	);
	let tier_2_options_open = $state(false);
	let tier_3_options_open = $state(false);
	let tier_4_options_open = $state(true);
	let after_remove_secondary_class: (() => void) | undefined = $state();
			...
			<div class="flex flex-col gap-4" bind:clientWidth={width}>
				<div class="flex flex-col gap-2 rounded-md bg-primary/50 p-2">
					...
					<DomainCardSelector
						...
						available_cards={get_available_domain_cards(characterCtx, level, 10, true)}
				...
				<Select.Root
					...
					onOpenChange={(open) => {
						if (open) {
							...
							tier_3_options_open =
								(choices.A.option_id?.startsWith('tier_3') ?? false) ||
								(choices.B.option_id?.startsWith('tier_3') ?? false);
				...
				{#each ['A' as keyof typeof choices, 'B' as keyof typeof choices] as key}
					{#if choices[key].option_id === 'tier_2_traits' || choices[key].option_id === 'tier_3_traits' || choices[key].option_id === 'tier_4_traits'}
						<TraitsSelector
							...
							marked_traits={derived_character_data.tier_4_marked_traits}
							{width}
						/>
