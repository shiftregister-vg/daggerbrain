<script lang="ts">
	...
	let {
		open = $bindable(false),
		instance = null,
		condition_list = $bindable([]),
		encounter_adversary_instances = [],
		disabled = false
	}: {
		open?: boolean;
		instance?: AdversaryInstance | null;
		condition_list?: string[];
		encounter_adversary_instances?: AdversaryInstance[];
		disabled?: boolean;
	} = $props();

	let newConditionName = $state('');
	let moreInfoOpen = $state(false);
	const canEdit = $derived(!disabled);
	const customConditions = $derived(
		condition_list.map((conditionName) => ({
			name: conditionName,
			enabled:
				instance?.conditions.some(
					(condition) => condition.name === conditionName && condition.enabled
				) ?? false
		}))
	);
	const canAddCustomCondition = $derived.by(() => {
		...
		if (DEFAULT_CONDITIONS.some((condition) => condition.name === name)) return false;
		return !condition_list.some((condition) => condition === name);
	});

	function setConditionEnabled(conditionName: Condition['name'], enabled: boolean) {
		if (!instance) return;

		if (instance.conditions.some((condition) => condition.name === conditionName)) {
			instance.conditions = instance.conditions.map((condition) =>
		...
		instance.conditions = [...instance.conditions, { name: conditionName, enabled: true }];
	}

	function removeCondition(conditionName: Condition['name']) {
		condition_list = condition_list.filter((condition) => condition !== conditionName);
		for (const adversaryInstance of encounter_adversary_instances) {
			adversaryInstance.conditions = adversaryInstance.conditions.filter(
			...
			{#if instance}
				{#each DEFAULT_CONDITIONS as condition}
					...
					>
						...
						<Switch
							disabled={!canEdit}
							class={cn('data-[state=unchecked]:bg-muted', !canEdit && 'pointer-events-none')}
							id={`condition-${condition.name}`}
							onCheckedChange={(checked) => {
								setConditionEnabled(condition.name, checked);
							}}
							checked={(instance.conditions ?? []).some(
								(existingCondition) =>
				...
				{#if canEdit}
					<div class="mt-6 flex items-center gap-2">
						<Input
							...
							class="flex-1"
							onkeydown={(event) => {
								if (event.key !== 'Enter' || !canAddCustomCondition) return;

								condition_list = [...condition_list, newConditionName.trim()];
								newConditionName = '';
							}}
						/>
						<Button
							...
							onclick={() => {
								if (!canAddCustomCondition) return;
					...
					<p class="mt-2 text-xs text-muted-foreground">
						* Deleting a custom condition will remove it from all adversaries in the encounter
					</p>
				{/if}
			{/if}
