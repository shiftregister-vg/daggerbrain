<script lang="ts">
	...
	import {
		...
		artForge,
		artStormy,
		screenshotCampaignDashboard
	} from '$lib/assets/images';
	import DeviceMockup from '$lib/components/decorations/device-mockup.svelte';
		...
		<div class="flex flex-col items-center sm:items-start">
			...
			>
				...
				<div class="flex flex-col items-center gap-6 text-center sm:items-start sm:text-left">
					...
					<p class="text-sm">Easily manage your Daggearheart games as a Player or GM.</p>
					<Button
						variant="outline"
						href="/roadmap"
			...
			<section class="mx-auto w-full max-w-5xl px-4 pt-20 md:px-8">
				<a
					href="/campaigns"
					class="campaigns-scroll-trigger group relative flex flex-col overflow-hidden rounded-xl border-x border-accent/0 text-left transition-all duration-600 md:max-h-[400px] md:flex-row"
				>
					<div class="relative w-full grow overflow-hidden">
						<enhanced:img
							src={screenshotCampaignDashboard}
							alt="Character sheet screenshot"
							sizes="(min-width: 768px) 50vw, 100vw"
							class="h-full w-full object-cover object-top transition-all duration-600"
						/>
					</div>
					<div
						class="relative z-20 flex grow flex-col justify-center gap-3 px-6 pt-5 pb-6 sm:w-1/2 sm:py-8"
					>
				...
				>
					...
					<div class="absolute inset-0">
						<enhanced:img
							...
							alt=""
							sizes="100vw"
							class="h-full w-full object-cover opacity-30 transition-all duration-600 group-hover:scale-103 group-hover:opacity-50"
							loading="lazy"
						/>
					</div>
					...
					>
						<div class="flex flex-col items-center gap-8 sm:text-left">
							...
							<div
								class={cn(buttonVariants(), 'pointer-events-none mx-auto w-min shrink-0 sm:mx-0')}
							>
								Create Homebrew
								<ArrowRight
									class="size-3.5 transition-transform duration-300 group-hover:translate-x-1"
