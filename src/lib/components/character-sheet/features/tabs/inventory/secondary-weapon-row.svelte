<script lang="ts">
	...
	const isEquipped = $derived(
		derived_character_data?.derived_secondary_weapon?.inventory_id === inventory_id
	);

	// Prefer the derived equipped weapon so visible rolls match active feature effects.
	const weapon = $derived(
		(isEquipped ? derived_character_data?.derived_secondary_weapon : undefined) ??
	...
	const inventoryItem = $derived(
		(character?.inventory.secondary_weapons ?? []).find((w) => w.inventory_id === inventory_id)
	);
	...
	function toggleEquip(e: MouseEvent) {
		...
		if (isEquipped) {
			...
			characterCtx.equipItem('secondary_weapon', inventory_id);
		}
	}
	...
	const willUnequipPrimaryWeapon = $derived.by(() => {
		...
		if (!derived_character_data.derived_primary_weapon) return false;

		const primaryBurden = derived_character_data.derived_primary_weapon?.burden ?? 0;
		return primaryBurden + weapon.burden > derived_character_data.max_burden;
	});

	const equipTitle = $derived.by(() => {
		if (isEquipped) return 'Unequip';
		if (willUnequipPrimaryWeapon) return 'Equip (unequips primary weapon)';
		return 'Equip';
	});
</script>
			...
			<div class="flex items-center gap-1">
				...
				<div class="-mb-1">
					...
					<p class="flex items-center gap-1.5 text-[10px] text-muted-foreground">
						...
						{/if}
						<span>Tier {level_to_tier(weapon.level_requirement)} secondary</span>
					</p>
				</div>
			</div>
		</td>
